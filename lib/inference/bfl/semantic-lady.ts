...
import { getBflSemanticModel, hasBflModelConfig } from './family';
...
export function assertBflSemanticParams(
  modelIdentifier: SherinModelId,
  params: SemanticParams,
) {
  if (!hasBflModelConfig(modelIdentifier)) {
    throwInvalidBflInput(
      `Semantic Lady does not define Black Forest Labs model ${modelIdentifier}.`,
    );
  }

  const model = getBflSemanticModel(modelIdentifier);
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    if (valueIssue) {
      throwInvalidBflInput(valueIssue);
    }
  }
  ...
  for (const field of model.schema) {
    ...
    throwInvalidBflInput(`${field.name} is required.`);
  }
}
...
function throwInvalidBflInput(message: string): never {
