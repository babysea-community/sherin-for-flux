...
const SHERIN_REPOSITORY_URL =
  'https://github.com/babysea-community/sherin-for-flux';
const SHERIN_VERCEL_DEPLOY_URL =
  'https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fbabysea-community%2Fsherin-for-flux&project-name=sherin-for-flux&repository-name=sherin-for-flux&env=NEXT_PUBLIC_SITE_URL,OWNER_EMAIL,NEXT_PUBLIC_SUPABASE_URL,NEXT_PUBLIC_SUPABASE_PUBLIC_KEY,SUPABASE_SECRET_KEY,INFERENCE_PROVIDER,BFL_API_KEY,BFL_API_BASE_URL,STORAGE_PROVIDER,CUSTOM_USER_STORAGE_QUOTA_GB';
...
const SHERIN_RAILWAY_DEPLOY_URL =
  'https://railway.com/deploy/sherin-for-flux?referralCode=_FJpRb';
...
function checkDeployButtons() {
  ...
  if (!readme.includes(expectedVercelButton)) {
    ok = false;
    fail(
      'README Vercel deploy button must clone babysea-community/sherin-for-flux.',
    );
  }
  ...
  if (!readme.includes(expectedDigitalOceanButton)) {
    ok = false;
    fail(
      'README DigitalOcean deploy button must clone babysea-community/sherin-for-flux from main.',
    );
  }
  ...
  if (!readme.includes(expectedNetlifyButton)) {
    ok = false;
    fail(
      'README Netlify deploy button must clone babysea-community/sherin-for-flux.',
    );
  }
  ...
  if (!readme.includes(expectedRenderButton)) {
    ok = false;
    fail(
      'README Render deploy button must clone babysea-community/sherin-for-flux.',
    );
  }
  ...
  for (const expected of [
    ...
    'repo_clone_url: https://github.com/babysea-community/sherin-for-flux.git',
